function initGenericSwipers() {
    $(".tf-swiper:not(.swiper-initialized)").each(function (index, element) {
        var $this = $(element);
...
window.initSwipers = function() {
    ...
    if ($(".section-testimonials").length > 0) {
        var sliderMain = new Swiper('.sw-main-image', {
            spaceBetween: 0,
            slidesPerView: 1,
            effect: "fade",
            fadeEffect: {
    ...
    if ($(".swiper-progressbar").length > 0 && !$(".swiper-progressbar").hasClass("swiper-initialized")) {
